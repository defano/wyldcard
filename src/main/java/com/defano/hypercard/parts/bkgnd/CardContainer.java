...
public interface CardContainer extends PartContainer {

    List<CardModel> getCardModels();

    default PartModel findCard(PartSpecifier ps) throws PartException {
        if (ps instanceof PartPositionSpecifier) {
            return findPartByPosition((PartPositionSpecifier) ps);
        } else {
            return findPart(ps, (List<PartModel>) (List<?>) getCardModels());
        }
    }
